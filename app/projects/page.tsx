...
export default async function HomePage() {
  ...
  const projectList = ((projects as Project[]) ?? []).map((project) => {
  ...
  const beforeCount = projectList.filter(
    (project) => project.status === "未着手" || project.status === "着工待ち"
  ).length;
    ...
    <main className="p-4">
      <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <Link
            href="/"
            className="rounded bg-gray-200 px-4 py-2 text-center font-semibold text-gray-800"
          >
            ←メニュー
          </Link>
